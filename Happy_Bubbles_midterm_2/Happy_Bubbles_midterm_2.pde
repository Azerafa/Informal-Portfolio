...
import ddf.minim.*;  //added
import ddf.minim.ugens.*; //added

Minim       minim;  //added
AudioOutput out;  //added
Oscil       wave;  //added
...
void setup()
{
  ...
  size(900, 700, P3D);  //added
  minim = new Minim(this);  //added
  out = minim.getLineOut();
  wave = new Oscil( 440, 0.5f, Waves.SINE );
  wave.patch( out );
  
  size(900, 700, P3D); // OpenGL mode
  ...
  rad = height/4.; // compute radius for central circle  tried to make one withing another..didn't work
  background(255); // clear the screen
...
void draw()
{
  ...
  if (!trace) background(0); // clear screen if showing geometry
  if (!trace) {
    stroke(0, 255,200,75); // black pen to turquoise pen..spelling
    fill(100,100,200); // don't fill to fill, odd color 
  }  
