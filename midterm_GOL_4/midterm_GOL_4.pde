...
void setup()
{
  size(700, 700); // make the screen square
...

{
  ...
  if (mousePressed)
  {
    ...
    c[which].pixels[my*w + mx] = color(170, 255, 110);
  }
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      {
        c[1-which].pixels[i*w + j] = color(170, 255, 110);   //green ones are dead 
      } else if (center==0 && sum==3) // dead... become alive
      {
        c[1-which].pixels[i*w + j] = color(175, 200, 255);  //blue are alive 
